...
//        ViewHolder holder = (ViewHolder) convertView.getTag();
        ...
        if (conversation.getAllMsgCount() != 0) {
            ...
            if(lastMessage.getType().name().equals(EMMessage.Type.TXT.name())) {
                EMTextMessageBody txtBody = (EMTextMessageBody) lastMessage.getBody();
                if(txtBody.getMessage().startsWith("WtwdMissionTxt:")){
                    String[] splits = txtBody.getMessage().split(":");
                    if(splits.length == 3){
                        holder.message.setText(splits[1]+"");
                        holder.name.setText("任务消息");
                        convertView.setTag(splits[2]);
                    }
                }
            }
