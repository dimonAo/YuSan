...
public class EaseMessageAdapter extends BaseAdapter{
    ...
    private static final int MESSAGE_TYPE_SENT_RED_PACKET = 14;
    private static final int MESSAGE_TYPE_RECV_RED_PACKET = 15;
    private static final int MESSAGE_TYPE_SENT_TASK = 16;
    private static final int MESSAGE_TYPE_RECV_TASK = 17;
    ...
    public int getItemViewType(int position) {
        ...
        if(customRowProvider != null && customRowProvider.getCustomChatRowType(message) > 0){
            ...
            Log.e("body",customRowProvider.getCustomChatRowType(message)+"");
        ...
        if (message.getType() == EMMessage.Type.TXT) {
            ...
            if(splits.length == 3) {
                if (splits[0].equals(WTWD_REDPACKET_TXT)){
                    return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_RED_PACKET : MESSAGE_TYPE_SENT_RED_PACKET;
                }
                if(splits[0].equals(WTWD_MISSION_TXT)){
                    return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_TASK : MESSAGE_TYPE_SENT_TASK;
                }
            }
                ...
                if(message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)){
                    ...
                    switch (getItemViewType(position)) {
                        ...
                        case MESSAGE_TYPE_RECV_RED_PACKET:
                            presenter = new RedPacketInfoCallPresenter();
                            break;
                        case MESSAGE_TYPE_SENT_TASK:
                        case MESSAGE_TYPE_RECV_TASK:
                            presenter = new MissionInfoCallPresenter();
                            break;
                        default:
